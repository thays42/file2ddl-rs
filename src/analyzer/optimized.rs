...
use crate::perf::{PerfMetrics, BufferOptimizer, StreamingOptimizer};
...
impl OptimizedAnalyzer {
    ...
    pub fn analyze_file(
        ...
        let (file_size, estimated_rows, estimated_columns) = self.analyze_file_structure(file_path, delimiter)?;
        
        if self.verbose {
            info!("File analysis: size={} bytes, ~{} rows, ~{} columns", 
        ...
        let chunk_size = StreamingOptimizer::calculate_chunk_size(estimated_rows, estimated_columns);
        
        if self.verbose {
            info!("Optimization settings: buffer={} bytes, chunk_size={} rows", 
                  optimal_buffer, chunk_size);
        }

        // Estimate memory requirements
        let estimated_memory = StreamingOptimizer::estimate_memory_for_analysis(estimated_rows, estimated_columns);
        if estimated_memory > available_memory / 2 { // Use half available memory as warning threshold
            warn!("Analysis may require significant memory: {} bytes estimated", estimated_memory);
        }
        ...
        // Run the analysis
        let result = engine.analyze_csv_file(file_path, delimiter, quote)?;
    ...
    fn analyze_file_structure(&self, file_path: &str, delimiter: u8) -> Result<(u64, usize, usize)> {
        ...
        let estimated_rows = if line.len() > 0 {
...
impl PerformanceTester {
    ...
    pub fn run_regression_tests() -> Result<()> {
        ...
        for (rows, cols, name) in test_cases {
            ...
            let _results = analyzer.analyze_file(
            ...
            if duration > max_duration {
                warn!("Performance regression detected for '{}': {:?} > {:?}", 
