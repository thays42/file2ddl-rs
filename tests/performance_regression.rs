use file2ddl::analyzer::optimized::{OptimizedAnalyzer, PerformanceTester};
...
fn test_column_scaling_performance() {
    ...
    for cols in column_counts {
        ...
        assert!(duration < max_duration,
                "Analysis with {} columns took too long: {:?}", cols, duration);
    }
}
...
fn test_type_inference_performance() {
    ...
    for (name, test_csv) in test_cases {
        ...
        assert!(duration < Duration::from_secs(1),
                "Type inference test '{}' took too long: {:?}", name, duration);
    }
}
...
fn test_performance_regression_suite() {
    ...
    assert!(result.is_ok(), "Performance regression tests failed: {:?}", result);
}
